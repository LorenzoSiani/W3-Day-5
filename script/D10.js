...
console.log('Javascript un po meno ahahah', me)
...
console.log(splitMe("sorry for benchmark"));
...
const deleteOne = function (boolean, string){
if(boolean === true){
  console.log(string.slice(0, string.lenght))
}else if (boolean === false){
  console.log(string.pop(string))
}
}

//deleteOne(true, string)
...
const onlyLetters = function (stringa){
  const newString = stringa.replace(/[0-9]/g, "")
  return newString
}

console.log(onlyLetters("love 2000 you"))
...
const myDate = new Date(1998, 12, 11);
const daysPassed = howManyDays(myDate);
console.log("Giorni trascorsi dalla mia nascita:", daysPassed)
...
const isTodayMyBirthday = function (MyBirthMonth,myBirthDay){ 
  
  let date = new Date () 
  let today = date.getDate ()
  let thisMonth = date.getMonth () + 1 
 
  if (today === myBirthDay && thisMonth === MyBirthMonth) {
...
return 'false'
 ...
 console.log (isTodayMyBirthday(12, 11))
...
// Ho spostato tutto l'array dei film in cima evitandolo poi di dovermelo richiamare
...
// l'ho provato diverse volte ma ricevevo sempre errore in console
...
const searchByTitle = function (movies, stringa) {
  ...
  movies.forEach((titolo) => {
    if (titolo.Title.includes(stringa)) {
      console.log(movies.Title); //non sono riuscito a capire come mai esce l'undefined in console
      return movies.Title;
    }
  });
};
searchByTitle(movies, "War");
...
//Alcuni di questi esercizi ricordavo fossero simili ad inizio settimana quindi ho preso spunto da lì
...
const removeTagLi = function () {
 
  const myUl = document.getElementById('myList') 
  
  let allLis= document.querySelectorAll ('#myList li')
   allLis.forEach ((e=> {
  myUl.removeChild (e)
   }))
}
...
console.log(halfTree(23));

//Per questo qui mi ha aiutato il mio caro amico GOOGLE  :)
